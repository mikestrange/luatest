...
local SOCKET_TICK_TIME = 0.5 			-- check socket data interval
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	if addrinfo == nil then return end
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		-- print("lua use memory = ", collectgarbage("count"), " k")
		-- luaoc.callStaticMethod(IosHelperCode.SystemHelper,"reportMemory",nil)
		while true do
...
-- function SocketTCP:stop_tick_event()
-- 	-- body
-- 	scheduler.unscheduleGlobal(self.tickScheduler)
-- 	self.tickScheduler = nil
-- end

-- function SocketTCP:start_tick_event()
-- 	-- body
-- 	self.tickScheduler = scheduler.scheduleGlobal(handler(self, self.tick_event), SOCKET_TICK_TIME)--__tick, SOCKET_TICK_TIME)
-- end

-- function SocketTCP:tick_event()
-- 	-- body
-- 	while true do
-- 			-- if use "*l" pattern, some buffer will be discarded, why?
-- 			local __body, __status, __partial = self.tcp:receive("*a")	-- read the package body
